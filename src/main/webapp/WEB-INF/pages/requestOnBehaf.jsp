...
<script>
	$(document).ready(function() {
        ...
        var sapUserIdlocal="";
			function handleData(data) {
				...
				if(!isEmpty(sodUserRequests )&& !isEmpty(sodUserRequests.requestId ))	{
					...
					console.log(sodUserRequests.sapUserId);
					$("#sapUserid").val(sodUserRequests.sapUserId);
		...
		.ajax({
			type : 'GET',
			//url : 'getOtherUsersDetails',
			url : 'searchOtherUsers/null',
			dataType : "json",
			async : false,
			success : handleOtherUsersDetails,
			error : function(xhRequest, ErrorText,
					thrownError) {
				//alert("Not Save");
				console.log(xhRequest.status);
       ...
       $('#usersOthers').on(
    			...
    			function() {
    				...
    				$.ajax({
    					type : 'GET',
    					url : 'searchOtherUsers/' + this.value,
    					dataType : "json",
    					async : false,
    					success : handleOtherUsersDetails,
    					error : function(xhRequest, ErrorText,
...
function handleDataUsers(data) {
 ...
 userdata = data;
   ...
   function handleOtherUsersDetails(data) {
		
	   userOthersdata = data;
	   userList = userOthersdata.map(function(code) {
	  		return {
	  			"text" : code.userDesc + ", "
	  					+ code.sapUserid,
	  			"value" : code.sapUserid
	  	...
	  	console.log(JSON.stringify(userOthersdata));
	  	$('.usersOthers').immybox('setChoices', userList);
	  	//sodUserRequests.sapUserId
	  $('.usersOthers').immybox('setValue', $("#sapUserid").val());
	      //console.log(JSON.stringify(sapUserIdlocal));
	 };
     ...
     $('.sapCompanyCodes').on('change', function() {
		...
		$('#bplant').click(
				function() {						
					...
					if(isEmpty($("#usersOthers").immybox('getValue')[0])){								
						swal("Select user",
						"Select user .");
						return;
					   }
					var result = userOthersdata.filter(obj => {
						  return obj.sapUserid === $("#usersOthers").immybox('getValue')[0];
				    })
         ...
         console.log(JSON.stringify(getFormData($("#requestFormTocde"))));
			...
			$
					.ajax({
						...
						type : 'POST',
						url : 'createTcodeRequestOnBehalf',
						data : JSON
					...
					function() {	
						...
						$
								.ajax({
									contentType : 'application/json; charset=utf-8',
									type : 'POST',
									url : 'createEmpRefRequestOnBehalf',
									data : JSON
		...
		<div class="portlet">
			...
			<div id="requestTocde">
				<div class="portlet">
					<form id="requestFormTocde">
						<div class="portlet-content form-group">
							...
							<input
								type="hidden" id="sapUserid" name="sapUserid" />
							 <input
								type="hidden" id="sapCompanyCode" name="sapCompanyCode" /> <input
								type="hidden" id="sapModuleCode" name="sapModuleCode" /> <input
								type="hidden" id="surRecid" name="surRecid" /> <input
								type="hidden" name="requestType" value="T" /> <input
								type="hidden" id="pgroups" name="purchaseGroup" /> <input
								type="hidden" id="plants" name="plant" /> <input type="hidden"
								id="tcodesText" name="tcode" />
							<table
								...
								<tr style="width: 100%">
									...
									<td style="width: 10%"><label for="usersOthers">Search
											Users</label></td>
									<td style="width: 40%"><input id="usersOthers"
										name="usersOthers" placeholder="Select users"
										class="form-control immybox immybox_witharrow usersOthers"
										type="text" value=""></td>
								</tr>
								<tr style="width: 100%">
								...
								<tr style="width: 100%">
									...
									<td style="width: 10%"><label for="requestId">Request
											Number</label></td>
									<td style="width: 40%"><input name="requestId"
										class="form-control" id="requestId" readonly="readonly" /></td>
									<td style="width: 10%">Reason for t-code access <font
										color=red>★</font></td>
									<td style="width: 40%"><input name="reason"
							...
							<div class="showback">
								...
								<button id="bPurGrp" type="button"
									class="btn btn-success  vpgroup" style="display: none;">
				...
				<div class="col-lg-12 column" style="width: 60%">
					<div class="portlet">
						<div class="portlet-header">
							<span class="ui-text">T-code added for approval <font
								color=red>★</font></span>
							<!--  <mark></mark> -->
						</div>
						<div class="portlet-content form-group">
				...
				<div class="col-lg-12 column" style="width: 40%">
					<div class="portlet">
						<div class="portlet-header">
							<span class="ui-text">Plant<font color=red>★</font>/Purchase
								Group List </font></span>
							<!--  <mark></mark> -->
						</div>
						<div class="portlet-content form-group">
							<div class="showback"></div>
			...
			<div id="requestRole" style="display: none">
					...
					<form id="requestFormRoleFrom">
						<div class="portlet-content form-group">
							<input type="hidden" id="roleId" name="roleId"> <input
								type="hidden" id="derivedRolesText" name="roleCode"> <input
							...
							<table
								...
								<tr style="width: 100%">
									<td style="width: 10%">Reason for role access <font
										color=red>★</font></td>
									<td style="width: 10%" colspan="9"><input
										name="reasonrole" class="form-control" id="reasonrole"
				...
				<div class="col-lg-12 column" style="width: 100%">
					<div class="portlet">
						<div class="portlet-header">
							<span class="ui-text">Role added for approval <font
								color=red>★</font>
							</span>
							<!--  <mark></mark> -->
						</div>
			...
			<div id="requestEmployeeRefrnces" style="display: none">
				<form id="requestEmployeeRefrncesFrom">
					<div class="portlet-content form-group">
						...
						<input type="hidden" id="refEmpCode" name="refEmpCode"> <input
							type="hidden" id="hreason" name="reason"> <input
							type="hidden" id="EmpSurRecid" name="surRecid"> <input

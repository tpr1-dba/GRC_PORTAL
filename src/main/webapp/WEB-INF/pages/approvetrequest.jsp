			...
			.ready(
									...
									if ($(this).hasClass('selected')) {												
										...
										if ($(this).hasClass('odd-row')) {
										 ...
										 seletedrequest=data;
											 ...
											 getAssignedRole(seletedrequest);
				  ...
				  $('#userRequestDataTable').on('click', 'button#btnHRApprove', function () {
							   // alert("Hello "+seletedrequest);
							    var closestRow = $(this).closest('tr');
							    var data = userRequestDataTable.row(closestRow).data();
							    var cdata =assignedRoleDataTables.rows().data();
							    ...
							    console.log( $("#"+seletedrequest.requestId).val());
							    if(cdata.length>=0 && cdata[0].isConflicted==='Y' ){
							    	//console.log(cdata[0]);							   
							    	 getConfilctsaved(data.requestId);							    	
								    return;
				              ...
				              if (isConfirm) {*/
								...
								//return;
								 ...
								 saveAssignedRoles(sdata);
					             }
					           });   
								
							});
						 function	getConfilctsaved(data){
								conflictData=null;
								$('#pageblock').modal({backdrop:'static',keyboard:false, show:true});
								$.ajax({
                  ...
                  function	openSaveRoleModel(data){            	   
									...
									.ajax({
										...
										success : function(data) {
											console
													.log(JSON
															.stringify(data));
											userRequestDataTable.ajax.url("getHodRequest").load();
											 $("#showRequestType").hide();
											  $('#pageblock').modal('hide');
											 swal("Success!",
													data.ok,
					...
					function getAssignedRole(seletedrequest){
						if(!isEmpty(seletedrequest)){
							assignedRoleDataTables.ajax.url("getAssignedRole/"+seletedrequest.requestId+"/"+seletedrequest.surRecid).load();							
						}
					};
					...
					assignedRoleDataTables = $("#assignedRoleDataTables").DataTable({
						...
						//We will use below variable to track page number on server side(For more information visit: http://legacy.datatables.net/usage/options#iDisplayStart)
						"iDisplayStart" : 0,
						"sAjaxSource" : "getAssignedRole/0/0",
						"aaSorting" : [ [ 0, "asc" ] ],	
						"aaSorting" : [],
						"columnDefs" : [ {
							"orderable" : false,
				...
				$('#assignedRoleDataTables').on('click', 'button#delete', function () {	
						...
						$
								.ajax({
									...
									success : function(data) {
										...
										if(!isEmpty(data)){
											assignedRoleDataTables.ajax.url("getAssignedRole/"+requestId+"/"+surRecid).load();							
										}
										  $('#pageblock').modal('hide');
											swal("Success!",
													data.ok,
					...
					$('#assignedRoleDataTables_wrapper #btnApproveRoles').on('click',
					   ...
					   // alert("Hello "+seletedrequest);
						...
						//alert("hello 2");
						saveAssignedRoles(rows_selected[0])
																	
					});
			
					
					function	saveAssignedRoles(data){	
						var requestId =data.requestId; 
						var surRecid=data.surRecid;
						//alert("hello 3");
						$('#pageblock').modal({backdrop:'static',keyboard:false, show:true});
						resetErrors();
						$
		...
		function	getTcodeByRoles(data){
			...
			$('#roleTcodeModal').on('hidden.bs.modal', function() {
				...
				$('#btSaveRoles').on('click',
				   ...
				   var sdata=	{};
		  		    sdata.status="A";
					sdata.requestId= seletedrequest.requestId;
					sdata.surRecid=  seletedrequest.surRecid;
					sdata.swmRecid=  seletedrequest.swmRecid;
					sdata.isSOD=isSOD;
					if(isSOD==="Y")
				    sdata.confilcteds=conflictData;	
				...
				$('#btnShowSOD').on('click',
							...
							if(isEmpty(seletedrequest)){								
							  ...
							  getConfilct(seletedrequest.requestId) ;
						...
						<table id="assignedRoleDataTables"
							...
							<thead>
								<tr>
									...
									<th style="width: 10%">Purchase Group</th>

...
@Repository("basisApproveDao")
public class BasisApproveDaoImpl extends JpaDao<SapAmUserAssignedRole> implements BasisApproveDao {
	static final Logger log = Logger.getLogger(BasisApproveDaoImpl.class.getName());
...
//					&& principal.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_ADMIN"));
			...
			queryBuilder.append(" and  ur.status not in ('R','C')   ");			
			...
			queryBuilder.append(" where ra.emp_code=:pempcode and  ra.IS_CTM='B' and ra.status='S' ");
	...
	public List<SapAmUserAssignedRole> getAssignedRole(HeapUserDetails principal, long requestId, long surRecid,
		...
		try {
			...
			System.out.println(requestId + "   surRecid   " + surRecid + "  " + criteria.iTotalRecords);
	...
	@Override
	public long updateAssignedRolesBasis(AssignedRolesVO assignedRolesVO, HeapUserDetails principal) throws Exception {
		// TODO Auto-generated method stub
		Session session = currentSession();
	...
	public long saveAssignedRoles(AssignedRolesVO assignedRolesVO, HeapUserDetails principal) throws Exception {
		...
		try {
			...
			// queryBuilder.append(" , ur.status = :pstatus"); STATUS WF_LEVEL APPROVED_ON
			// APPROVED_BY
			queryBuilder.append(" WHERE   ur.status = 'A'  ");
			queryBuilder.append(" and  ur.SUR_RECID = :psurRecid and ur.REQUEST_ID = :prequestId ");

...
@Repository("requestHistoryDao")
public class RequestHistoryDaoImpl extends JpaDao<SapAmUserRequest> implements RequestHistoryDao {
	static final Logger log = Logger.getLogger(RequestHistoryDaoImpl.class.getName());
    ...
    public List<SAPUserRequestVO> getUserRequest(JQueryDataTableParamModel criteria, HeapUserDetails principal)
    	...
    	try {
			...
			queryBuilder.append(" ur.sap_user_id as sapUserid , ur.reason as reason, ur.status as status, ur.SAP_MODULE_CODE as sapModuleCode, ur.WF_LEVEL as wfLevel, ur.REQUEST_DATE as appliedOn");
    ...
    @Transactional
	@Override
	public List<SapAmUserRequestApprovalVO> getUserRequestStatus(Long surRecid, Long requestId,JQueryDataTableParamModel criteria, HeapUserDetails principal) {
		// TODO Auto-generated method stub
		Session session = currentSession();
    	try {
			StringBuilder queryBuilder = new StringBuilder(" select  LISTAGG (rp.company_code, ', ') within  group (  order by rp.request_id) AS companyCode, e.employee_name AS empCode,  ");			
			queryBuilder.append(" LISTAGG (rp.SAP_MODULE ,', ') within    group (  order by rp.request_id) as sapModule ,  ");
			queryBuilder.append(" LISTAGG ( rp.sbu , ', ') within  group (  order by rp.request_id) AS sbu, rp.status AS status, rp.wf_code AS wfCode, rp.wf_level AS wfLevel, ");
			queryBuilder.append(" rp.emp_code AS  userCode,rp.request_id AS requestNo,rp.swm_recid AS sapAmRecId  ");
			queryBuilder.append("  FROM sap_am_user_request_approvals rp join hrm_employee E on rp.emp_code=e.emp_code  ");
			// added on 20-06-2023 for  and a.wf_code= rp.wf_code 
			queryBuilder.append(" where rp.request_id=:prequestId and rp.parent_recid =:psurRecid and rp.status ='S' and rp.wf_level= ( select max(a.wf_level) FROM "
					+ "        sap_am_user_request_approvals a where a.emp_code=rp.emp_code   and a.wf_code= rp.wf_code  and a.request_id= rp.request_id )  ");
			//queryBuilder.append(" group by rp.company_code,e.EMPLOYEE_NAME, rp.sbu,  rp.wf_level ,rp.status ,rp.wf_code  ");
			queryBuilder.append(" group by e.EMPLOYEE_NAME,  rp.wf_level ,rp.status ,rp.wf_code,rp.emp_code,rp.request_id,rp.swm_recid  ");

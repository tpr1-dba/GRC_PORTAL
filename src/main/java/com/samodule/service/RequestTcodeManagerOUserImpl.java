...
@Service("requestTcodeManagerOUser")
public class RequestTcodeManagerOUserImpl implements RequestTcodeManagerOUser {
	@Autowired
...
//		}
		...
		List<String> sbus = Arrays.asList("OUT");//masterDataDao.getSapPlantSbus(plants);
...
//				principal);
	...
	public SapAmUserRequestApproval getApprovelWorkFlowsBySWMRecid(long swmRecid,
		...
		SapAmUserRequestApproval reqApproval=null;
		try {
			List<SapAmUserRequestApproval> list = sapAmUserRequestApprovalDao.getApprovelWorkFlowsBySWMRecid(swmRecid, principal);
			if(!CollectionUtils.isEmpty(list)) {
				reqApproval = list.get(0);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
